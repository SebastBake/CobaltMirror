package com.unimelbit.teamcobalt.tourlist.Home;

import org.junit.Test;
import org.junit.runner.RunWith;
import org.robolectric.RobolectricTestRunner;
import org.robolectric.annotation.Config;

/**
 * Refer to the PostRequesterTest this class doesn't add much
 * application logic, just specifies a specific URL to send to
 * and data to send.
 */

@RunWith(RobolectricTestRunner.class)
@Config(manifest=Config.NONE)
public class RegisterUserPostRequestTest {

    /*
     * Place holder because class needs to contain a test
     *
     * Replace when/if tests are added.
     * i.e. if unique application logic is added to the class.
     */

    @Test
    public void stubTest() throws Exception {

    }

}